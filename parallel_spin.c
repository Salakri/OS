...
mutex        8.17   4.54   2.23   2.38   2.26   2.48   2.88
spin         8.10   5.15   3.57   2.43   2.23   2.19   2.33

Yes, there is a time change between the mutex and the spinlock. 

When the mutex meets the conflict, it uses the context switch, so one thread is blocked to not take the CPU usage. In the spinlock condition, the thread waits but it is not blocked like the mutex. It keeps spinning for the resource. 

From the time data above, the mutex generally uses less time than the spinlock. It is because the spinlock is busy-waiting which wastes CPU cycles compared to the mutex. When the number of threads increases, it makes the mutex longer time to go through the wait queue, thus the time increases compared to the spinlock.
