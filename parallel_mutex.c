...
//pthread_mutex_t lock; // declare a lock in part1
pthread_mutex_t lockarr[NUM_BUCKETS];
...
// Part 1: the competition of the resource is the reason for losing the keys
...
void insert(int key, int val) {
    ...
    pthread_mutex_lock(&lockarr[i]);   // acquire lock in part 4 
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;    
    pthread_mutex_unlock(&lockarr[i]);   // release lock in part 4
}

// Part 3: No, it does not require a lock, thus no need for any changes
...
void * put_phase(void *arg) {
    ...
    for (key = tid ; key < NUM_KEYS; key += num_threads) {
        //pthread_mutex_lock(&lock); // acquire lock in part1
        insert(keys[key], tid);
        //pthread_mutex_unlock(&lock); //release lock in part1
    }
...
int main(int argc, char **argv) {
    ...
    pthread_t *threads;
    double start, end;
    //pthread_mutex_init(&lock, NULL); // initialize the lock in part 1
